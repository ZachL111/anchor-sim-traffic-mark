...
`anchor-sim-traffic-mark` is centered on simulations work in C#. The technical target is: Create a C# reference implementation for traffic workflows, centered on stream reduction, windowed input fixtures, and late-data behavior checks.
...
The score is deterministic. The current threshold is 168. Risk has a penalty of 5, latency has a penalty of 3, and weight contributes a bonus of 4.
